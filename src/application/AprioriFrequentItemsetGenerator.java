...
public class AprioriFrequentItemsetGenerator<I> {
    ...
    public FrequentItemsetData<I> generate(List<Set<I>> transactionList, 
        ...
        if (minimumSupport > 1.0) {
            throw new IllegalArgumentException(
                    "support supérieur à 1: " + minimumSupport + ", " +
                    "changer le support");
        }
    ...
    private List<Set<I>>
        ...
        //System.out.println("map elment avant dernier (notre résultat)" + map.get(map.size()-1)); // le résultat
        return ret;
    }

    private void printResultedFrequentItemset(List<Set<I>> list) {
    	// Méthode itemset résultant
    	// list contains the frequent itemsets
    	System.out.println("map elment avant dernier (notre résultat)" + list);
