 ...
 */
public class BoxAndWhiskerDemo2 extends ApplicationFrame {
    ...
    public BoxAndWhiskerDemo2(final String title, String nomFichier) throws IOException {
		...
		String[] all_att=null;
		Instances datasett,datasetR, datasetR2 = null;
		...
		else
		{
			...
			{				
				...
				*/
				datasetR = MissValues_replace(datasett);
				
				// Après remplacement on fait la normalisation 
				datasetR2 = NormaliserDataset(datasetR);
		        ...
		        final BoxAndWhiskerCategoryDataset ddataset = createSampleDataset(datasetR2);
			...
			{		        
				datasetR = NormaliserDataset(datasett);
		        final BoxAndWhiskerCategoryDataset ddataset = createSampleDataset(datasetR);
	...
	public Instances NormaliserDataset(Instances datasett) {
		int nb_ligne, nb_att;
		String[] tabValeur;
		String[][] data;
		...
		{	
			...
			if(datasett.attribute(j).isNumeric() && !datasett.attribute(j).isDate())
			{
				...
				//	float val = Float.parseFloat(datasett.attribute(j));
					...
					if(max-min != 0)
					{
						...
						datasett.instance(l).setValue(datasett.attribute(j), v);
					}
				}
			}
		}
